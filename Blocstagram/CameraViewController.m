    ...
    [AVCaptureDevice requestAccessForMediaType:AVMediaTypeVideo completionHandler:^(BOOL granted) {
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            if (granted) {
                ...
                [alertVC addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"OK", @"OK buttuon") style:UIAlertActionStyleCancel handler:^(UIAlertAction *action) {
                    [self.delegate cameraViewController:self didCompleteWithImage:nil];
                }]];
...
- (void) cameraButtonPressedOnToolbar:(CameraToolBar *)toolbar {
    ...
    for (AVCaptureConnection *connection in self.stillImageOutput.connections) {
        ...
        // #9
        [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler:^(CMSampleBufferRef imageSamplerBuffer, NSError *error) {
            if (imageSamplerBuffer) {
                // #10
                NSData *imageData = [AVCaptureStillImageOutput jpegStillImageNSDataRepresentation:imageSamplerBuffer];
